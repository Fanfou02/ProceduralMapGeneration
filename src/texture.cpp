...
bool Texture::createSunBillboardTexture()
{
    ...
    **/
    ...
    const int HALLO_SIZE = width * 1/5;

    for (int col = 0; col < width; ++col) {
        for (int row = 0; row < height; ++row) {
            img[(row * width + col) * 4 + 0] = 255; // R
            img[(row * width + col) * 4 + 1] = 120; // G
            img[(row * width + col) * 4 + 2] = 0; // B

            const int distance_to_center = sqrt(pow(col - CENTER_X, 2) + pow(row - CENTER_Y, 2));
            int alpha = 255;
            if (distance_to_center >  HALLO_SIZE)
                alpha = 0;
            else if (distance_to_center > CIRCLE_RADIUS){
                alpha = 255 - 255*(distance_to_center - CIRCLE_RADIUS)/(HALLO_SIZE - CIRCLE_RADIUS);
            }
